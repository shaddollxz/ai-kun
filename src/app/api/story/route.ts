...
export async function POST(request: Request) {
  ...
  try {
    const voiceRes = await fetch('https://v1.reecho.cn/api/tts/generate', options)
...
function splitTextIntoParagraphs(inputText: string, voiceId: string): Array<{ text: string; voiceId: string }> {
